...
select	
	m.genre,
	SUM(rental_counts.num_rentals * m.renting_price) as movie_income
from movies as m 
join (
	select
		movie_id,
	...
	from renting
	group by movie_id
) as rental_counts on m.movie_id = rental_counts.movie_id
group by m.genre
order by movie_income desc;

	-- Top 3 movie genres with the highest rental income are: 1. Drama, 2. SF&Fantasy and 3. Comedy

select 
	m.title,
	rental_counts.num_rentals * m.renting_price as movie_income
from movies as m 
join (
	select
		movie_id,
...
order by movie_income desc;
	...
	-- The highest rental income is generated by the following movies: 1. 'Bridget Jones - The Edge of Reason', 2. 'Fair Game', 3. 'The Kingdom'.
...
-- Extra query for calculating aggregation functions but only for movies with three or more ratings
...
-- Another extra query (I just can't stop!!!) for calculating aggregation functions with OLAP methods
...
left join movies as m
on r.movie_id = m.movie_id
group by cube(m.genre, date_part('year', r.date_renting))
order by genre, year_of_renting;
